-- Fix register_volunteer_public to avoid referencing a non-existent constraint name
-- Use ON CONFLICT (phone) DO NOTHING so it can use any applicable unique index
-- This resolves errors like: constraint "unique_phone_non_empty" for table "contacts" does not exist
...
BEGIN
  ...
  -- Ensure a contact by phone (global)
  ...
  IF NOT FOUND THEN
    -- Insert owned by event creator; tolerate races via ON CONFLICT on phone
...
-- Ensure execute permissions remain for anon and authenticated
